...
export class DataProcessor {
  ...
  private static cachedIndex: {
    byCategory: Map<string, MCATTopic[]>
    byProvider: Map<string, MCATTopic[]>
    byType: Map<MCATTopic['type'], MCATTopic[]>
    highYieldByCategory: Map<string, MCATTopic[]>
  } | null = null

  /**
   * Creates optimized index for fast lookups
   */
  private static createIndex(topics: MCATTopic[]): void {
    const byCategory = new Map<string, MCATTopic[]>()
    const byProvider = new Map<string, MCATTopic[]>()
    const byType = new Map<MCATTopic['type'], MCATTopic[]>()
    ...
    const highYieldByCategory = new Map<string, MCATTopic[]>()
    topics.forEach(topic => {
   ...
   */
  private static getIndex(topics: MCATTopic[]): typeof DataProcessor.cachedIndex {
    if (!this.cachedIndex) {
   ...
   */
  public static getHighYieldTopics(topics: MCATTopic[], priorities: string[]): MCATTopic[] {
    const index = this.getIndex(topics)
    if (!index) return []

    const result: MCATTopic[] = []
   ...
   */
  public static getTopicsByCategory(topics: MCATTopic[], category: string): MCATTopic[] {
    const index = this.getIndex(topics)
   ...
   */
  public static getTopicsByProvider(topics: MCATTopic[], provider: string): MCATTopic[] {
    const index = this.getIndex(topics)
   ...
   */
  public static getTopicsByType(topics: MCATTopic[], type: MCATTopic['type']): MCATTopic[] {
    const index = this.getIndex(topics)
   ...
   */
  public static getTopicsForAnchor(
    topics: MCATTopic[],
    anchor: AnchorKey,
    ...
    if (anchor.level === 0 && anchor.concept) {
      ...
      candidates = candidates.filter(
        topic =>
          topic.category === anchor.category &&
      ...
      candidates = candidates.filter(
        topic => topic.category === anchor.category && topic.subtopic === anchor.subtopic
      )
    } else {
   ...
   */
  public static sortTopicsByCriteria(
    topics: MCATTopic[],
   ...
   */
  private static getSpecificityScore(topic: MCATTopic): number {
    if (topic.concept) return 0 // Most specific
    if (topic.subtopic) return 1 // Medium specificity
   ...
   */
  private static getNumericKeyOrder(topic: MCATTopic): number {
   ...
   */
  private static getProviderRank(provider: string): number {
    const providerLower = provider.toLowerCase()

    if (providerLower.includes('khan')) return 1
    if (providerLower.includes('kaplan')) return 2
    if (providerLower.includes('jack westin')) return 3
    if (providerLower.includes('uworld')) return 4
    if (providerLower.includes('aamc')) return 5
    return 6
  }
   ...
   */
  public static generateResourceUID(topic: MCATTopic): string {
   ...
   */
  public static isResourceUsed(topic: MCATTopic, usedResourceUIDs: Set<string>): boolean {
   ...
   */
  public static getAvailableHYTopics(
    allTopics: MCATTopic[],
    priorities: string[]
  ): Map<string, MCATTopic[]> {
    const available = new Map<string, MCATTopic[]>()

    priorities.forEach(priority => {
      const categoryTopics = this.getTopicsByCategory(allTopics, priority).filter(
        topic => topic.high_yield === 'Yes'
      )
    ...
    for (const [, topics] of availableTopics) {
      ...
      if (firstTopic) {
        return {
          ...
          level: (() => {
            if (firstTopic.concept) return 0
            if (firstTopic.subtopic) return 1
            return 2
          })(),
        }
      }
